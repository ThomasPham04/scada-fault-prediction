"""
RandomForestModel — models.architectures.random_forest
Factory class for the Random Forest binary classifier used in anomaly detection.
"""
...
class RandomForestModel:
    """
    Configures a Random Forest binary classifier for SCADA fault detection.

    The model ingests flattened (window × feature) vectors and outputs a
    probability in [0, 1]. Uses class_weight='balanced' by default, which
    ...
    def build(self) -> RandomForestClassifier:
        """
        Instantiate and return a configured (untrained) RandomForestClassifier.

        Returns:
            RandomForestClassifier ready to be fitted.
        """
        ...
        print(
            f"[RandomForest] Model configured: {self.n_estimators} trees, "
            f"depth={depth_str}, max_features={self.max_features}, "
